...
assert(zh.includes("half=(V-cardW-GP*2)/2") && zh.includes("tcW=Math.min(200,Math.round(half*5/13))") && zh.includes("lsW=Math.min(320,Math.round(half*8/13))"), "两侧按 5:8 比例分配")
assert(zh.includes("mn.style.marginLeft=showT?(((V-cardW)/2-tcW-GP-po)+'px'):'auto'"), "main margin-left 精确主卡居中（含 pageOffset）")
