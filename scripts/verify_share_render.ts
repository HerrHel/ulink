...
assert(zh.includes(".layout{display:flex;gap:24px;align-items:flex-start;justify-content:flex-start}"), "layout flex-start（dl margin-left 接管位置）")
assert(zh.includes("@media(max-width:1240px)"), "无 JS 兜底断点")
// ── 1.55 JS 动态布局：流内（toc sticky / 列表流内），main margin-left 精确主卡居中 ──
assert(zh.includes("function dl()") && zh.includes("cardW=Math.max(320,Math.min(660,Math.round(V*0.55)))"), "dl 动态主卡宽（视口 55% 封顶 660）")
assert(zh.includes("L=lay.offsetWidth||V") && zh.includes("po=(V-L)/2"), "layout 实际宽度 + pageOffset 修正")
assert(zh.includes("half=(V-cardW-GP*2)/2") && zh.includes("tcW=Math.min(200,Math.round(half*5/13))") && zh.includes("lsW=Math.max(0,Math.round(half-GP))"), "TOC 5:13 比例、列表占满右侧剩余（half-GP）")
assert(zh.includes("ml=(V-cardW)/2-tcW-GP-po"), "dl 计算 ml（主卡居中偏移）")
assert(zh.includes("tc.style.marginLeft=showT?(ml+'px'):''"), "TOC margin-left=ml → 紧贴主卡左侧")
assert(zh.includes("mn.style.marginLeft=showT?'0':(((V-cardW)/2-po)+'px')"), "main：showT 时 0（TOC 推位）/ 隐藏时精确值居中（不再 auto 防列表挤偏）")
assert(!zh.includes("tc.style.position='fixed'") && !zh.includes("ls.style.position='fixed'"), "toc/list 流内（不悬浮）")
assert(zh.includes("tc.style.display=showT?'':'none'"), "TOC 显隐（含 canScroll 短内容）")
