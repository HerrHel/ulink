...
/** 图标位渲染：优先 favicon/URL 图标（带 onerror 降级），否则首字母占位（grid 同格）。 */
...
/** favicon 降级脚本（渐进增强）：img[data-fb] 加载失败时隐藏自身，露出同格首字母。 */
...
serve(async (req) => {
  ...
  if (error || !data || !data.group) {
    const html = [
      ...
      `<div class="page">`,
      `<header class="head">`,
      `<a class="logo" href="${esc(APP_ORIGIN)}/">${LOGO_SVG}<span>${esc(dict.logoText)}</span></a>`,
      `</header>`,
      `<main class="main">`,
      `<div class="nf">`,
      ...
      `<a class="cta" href="${esc(APP_ORIGIN)}/">${esc(dict.backHome)}</a>`,
      `</div>`,
      `</main>`,
      `</div>`,
      `</body>`,
